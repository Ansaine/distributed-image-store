...
// http://127.0.0.2:8080/get?key=hello
func getHandler(w http.ResponseWriter, r *http.Request) {
	...
	db1 := newStore("db1.json")
	value, exists := db1.get(key)
	if !exists {
...
func setHandler(w http.ResponseWriter, r *http.Request) {
	...
	db1 := newStore("db1.json")
	db1.set(requestData.Key, requestData.Value)

	// Send back the response as JSON
	response := map[string]string{
		"message": "key set in db 1",
	}
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(response)
...
func main() {
	...
	fmt.Println("db-server-1 starting on 127.0.0.2",port)
	err := http.ListenAndServe("127.0.0.2"+port, nil)
	if err != nil {
		fmt.Println("error starting db-server-1 : ", err)
	}
}
